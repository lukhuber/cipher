...
export function updateBunkerRCL2(room: Room, anchorPos: RoomPosition): void {
	...
	for (const e of extensions) {
		room.createConstructionSite(anchorPos.x + e.x, anchorPos.y + e.y, STRUCTURE_EXTENSION);
	}

	// Place a container at every mining site (at the flags position) ---------------------------------------------------
	const miningSites: Flag[] = room.find(FIND_FLAGS, {
		filter: (f) => {
	...
	const upgradeSite: Flag = room.find(FIND_FLAGS, {
		filter: (f) => {
	...
	if (controller.level < 4) {
		// Don't place this container anymore, once level 4 is reached
		room.createConstructionSite(anchorPos.x + 1, anchorPos.y + 0, STRUCTURE_CONTAINER);
	}
}
...
export function updateBunkerRCL3(room: Room, anchorPos: RoomPosition): void {
	...
	room.createConstructionSite(anchorPos.x + tower.x, anchorPos.y + tower.y, STRUCTURE_TOWER);
	for (const r of roads) {
		room.createConstructionSite(anchorPos.x + r.x, anchorPos.y + r.y, STRUCTURE_ROAD);
	}
}
...
export function updateBunkerRCL4(room: Room, anchorPos: RoomPosition): void {
	...
	room.createConstructionSite(anchorPos.x + storage.x, anchorPos.y + storage.y, STRUCTURE_STORAGE);
	for (const r of roads) {
		room.createConstructionSite(anchorPos.x + r.x, anchorPos.y + r.y, STRUCTURE_ROAD);
	}
}
...
export function updateBunkerRCL8(room: Room, anchorPos: RoomPosition): void {
	...
	for (const t of towers) {
		room.createConstructionSite(anchorPos.x + t.x, anchorPos.y + t.y, STRUCTURE_TOWER);
	}
}
